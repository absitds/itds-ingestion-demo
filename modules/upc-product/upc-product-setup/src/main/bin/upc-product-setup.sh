...
. ${MODULE_HOME}/etc/bin/upc-product-setup.shell.properties
...
if fn_check_if_file_exists "${BATCH_ID_DATA_DIR}"/upc-product/"${BATCH_ID_FILE_NAME}"
then

  export BATCH_ID=$(cat "${BATCH_ID_DATA_DIR}"/upc-product/"${BATCH_ID_FILE_NAME}")

else

  fn_create_local_directory "${BATCH_ID_DATA_DIR}/upc-product/"
  fn_get_batch_id
  echo "${BATCH_ID}" > "${BATCH_ID_DATA_DIR}"/upc-product/"${BATCH_ID_FILE_NAME}"

fi

fn_log_info "Successfully Created Batch ID"

if fn_check_if_file_exists "${MIN_RECORD_DIR}/upc-product_min_record.value"
then
...
fn_write_val_to_file \
"$MAX_VAL" \
"${MAX_RECORD_DIR}/upc-product_max_record.value"
